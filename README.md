...
Azure DevOps provides analytics views and ODATA support for more advanced reporting scenarios.  These are powerful capabilities, but can also introduce challenges for organizations when reports are significantly large and complex and  start running into service level resource limits.  It's not uncommon to encounter throttling by the ADO analytics service, Power BI Service, or hit limitations of the Power BI client when running queries on a desktop.  You might find queries work on your desktop, but hit limits after they are published - or work from the Power BI Service but fail on the desktop due to memory exhaustion.  Sometimes they work for awhile in either environment and then hit some tipping point.  There are a bunch of combinations of resource limits you can find yourself trying to work around -- just to get your report to run.  It can also be hard to determine exactly when/where a limit is being hit.  Simply put, if you are doing large scale reporting with ADO, it can be a journey to navigate these types of issues.  Some things you control, some you cannot.
...
I've found myself wrestling with service level throttling enough to ponder the question -- "how can I just get the data somewhere I'm not fighting with the services and build reports any way I want?"
...
As of today (July 2025), while there is still a little bit of work involved to get ADO data into Fabric, it's a much more capable platform for ADO reporting needs.  The analytics endpoint for ADO does a pretty good job of exposing all the data.  You may still need to fine tune queries and date ranges to avoid throttling by the ADO analytics service during ingestion, but you have much more control of the compute in Fabric once it's in there.
...
While Fabric provides some capabilities for incremenetal refresh, it depends on data sources that support it properly.  As of now, the ODATA implementation with ADO isn't one of them, but you can incorporate dates into your Analytics queries and build some simple logic to effectively keep your queries small and implement a refresh that only grabs deltas. 
The other thing you will run into is that you may need to do some data conversion between ADO's ODATA results and what you can push into Fabric Warehouse. You can't just grab and dump.  For instance, Date/Time/Zome will need to be converted to Date/Time.  A lot of ODATA queries return records of entities you may need to break down into simpler types. For more details, check out https://learn.microsoft.com/en-us/fabric/data-factory/dataflow-gen2-data-destinations-and-managed-settings#supported-data-source-types-per-destination
 
The approach below uses a pipeline+dataflow to build a tracking table of ADO projects and store a LastModified data (when it was last imported).  It uses a second pipeline+dataflow to enumerate the tracking table and run an import of each ADO project to pull the latest workitems into Fabric. 

Before you start down this road, it's a good idea to spend some time exploring the Azure DevOps Analytics and ODATA implementation - along with query guidelines to get more familiar with the model and patterns that work against you.
##References:
